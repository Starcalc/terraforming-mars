...
export class LocalHeatTrapping implements IProjectCard {
    ...
    public canPlay(player: Player, game: Game): boolean {
        ...
        if (player.canUseHeatAsMegaCredits) {
            return (
                player.heat >= requiredHeatAmt &&
                player.heat + player.megaCredits >= requiredHeatAmt + player.getCardCost(game, this)
            );
        }

        return (
            player.heat >= requiredHeatAmt ||
            (player.isCorporation(CardName.STORMCRAFT_INCORPORATED) &&
                player.getResourcesOnCorporation() * 2 + player.heat >= 5)
        );
    }
