...
export class RoboticWorkforce implements IProjectCard {
    ...
    private miningSteelProduction = 0;
    private miningTitaniumProduction = 0;
    ...
    private getAvailableCards(player: Player, game: Game): Array<ICard> {
        ...
        const availableCards: Array<ICard> = player.playedCards.filter((card) => {
            for (let i = 0; i < builderCardsNames.length; i++) {
                if (
                    builderCardsNames[i] === card.name &&
                    card.name === CardName.BIOMASS_COMBUSTORS
                ) {
                    ...
                    builderCardsNames[i] === card.name &&
                    card.name === CardName.TROPICAL_RESORT
                ) {
                    ...
                    builderCardsNames[i] === card.name &&
                    card.name === CardName.HEAT_TRAPPERS
                ) {
                    ...
                    builderCardsNames[i] === card.name &&
                    (card.name === CardName.PEROXIDE_POWER ||
                    ...
                    builderCardsNames[i] === card.name &&
                    card.name === CardName.NUCLEAR_POWER
                ) {
                    ...
                    builderCardsNames[i] === card.name &&
                    card.name === CardName.FOOD_FACTORY
                ) {
        ...
        if (
            ...
            corporationCardNames.has(player.corporationCard.name)
        ) {
    ...
    public play(player: Player, game: Game) {
        ...
        return new SelectCard(
            ...
            (selectedCards: Array<ICard>) => {
                ...
                if (
                    foundCard.name === CardName.MINING_AREA ||
                    foundCard.name === CardName.MINING_RIGHTS
                ) {
                ...
                const updaters: Array<Updater> = [
                    ...
                    new Updater(
                        CardName.MEDICAL_LAB,
                        0,
                        Math.floor(player.getTagCount(Tags.STEEL) / 2),
                        0,
                        0,
                        0,
                        0
                    ...
                    new Updater(
                        CardName.GYROPOLIS,
                        -2,
                        player.getMultipleTagCount([Tags.VENUS, Tags.EARTH]),
                        0,
                        0,
                        0,
                        0
                    ...
                    new Updater(
                        CardName.PARLIAMENT_HALL,
                        0,
                        Math.floor(player.getTagCount(Tags.STEEL) / 3),
                        0,
                        0,
                        0,
                        0
