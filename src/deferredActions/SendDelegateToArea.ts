...
export class SendDelegateToArea implements DeferredAction {
    ...
    public execute() {
        ...
        if (this.replace) {
            parties = this.game.turmoil!.parties.filter((party) => {
                ...
                for (const delegate of party.delegates) {
                    ...
                    return (
                        party.delegates.filter((delegate) => delegate === this.replace).length > 1
                    );
                }
                return false;
            });
        ...
        sendDelegate.options = parties.map(
            (party) =>
                new SelectOption(
                    ...
                    () => {
                        ...
                        if (this.nbr > 1 && this.fromLobby) {
                            // For card: Cultural Metropolis
                            this.game.turmoil?.sendDelegateToParty(
                                this.player.id,
                                party.name,
                                this.game,
                                true
                            ...
                            for (let i = 0; i < this.nbr; i++) {
                                if (this.replace) {
                                    this.game.turmoil?.removeDelegateFromParty(
                                        this.replace,
                                        party.name,
                                        this.game
                                    );
                        ...
                        this.game.log("${0} sent ${1} delegate(s) in ${2} area", (b) =>
                            b.player(this.player).number(this.nbr).party(party)
                        );
                        return undefined;
                    }
                )
