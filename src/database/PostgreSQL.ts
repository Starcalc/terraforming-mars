...
export class PostgreSQL implements IDatabase {
    ...
    getClonableGames(cb: (err: any, allGames: Array<IGameData>) => void) {
        ...
        const sql =
            "SELECT distinct game_id game_id, players players FROM games WHERE save_id = 0 order by game_id asc";
    ...
    getGames(cb: (err: any, allGames: Array<string>) => void) {
        ...
        const sql =
            "SELECT games.game_id FROM games, (SELECT max(save_id) save_id, game_id FROM games WHERE status='running' AND save_id > 0 GROUP BY game_id) a WHERE games.game_id = a.game_id AND games.save_id = a.save_id ORDER BY created_time DESC";
        this.client.query(sql, (err, res) => {
            if (err) {
    ...
    restoreReferenceGame(game_id: string, game: Game, cb: (err: any) => void) {
        ...
        this.client.query(
            "SELECT game_id game_id, game game FROM games WHERE game_id = $1 AND save_id = 0",
            [game_id],
            (err: any, res) => {
                if (err) {
    ...
    restoreGameLastSave(game_id: string, game: Game, cb: (err: any) => void) {
        ...
        this.client.query(
            "SELECT game game FROM games WHERE game_id = $1 ORDER BY save_id DESC LIMIT 1",
            [game_id],
            (err, res) => {
                if (err) {
    ...
    cleanSaves(game_id: string, save_id: number): void {
        ...
        this.client.query(
            "DELETE FROM games WHERE game_id = $1 AND save_id < $2 AND save_id > 0",
            [game_id, save_id],
            (err) => {
                if (err) {
        ...
        this.client.query(
            "DELETE FROM games WHERE players = 1 and created_time < now() - interval '1 days'",
            (err: { message: any }) => {
                if (err) {
        ...
        this.client.query(
            "DELETE FROM games WHERE created_time < now() - interval '10 days'",
            (err: { message: any }) => {
                if (err) {
                    return console.warn(err.message);
                }
    ...
    restoreGame(game_id: string, save_id: number, game: Game): void {
        // Retrieve last save from database
        this.client.query(
            "SELECT game game FROM games WHERE game_id = $1 AND save_id = $2 ORDER BY save_id DESC LIMIT 1",
            [game_id, save_id],
            (err, res) => {
                if (err) {
