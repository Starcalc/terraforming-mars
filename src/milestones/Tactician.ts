...
export class Tactician implements IMilestone {
    public name = "Tactician";
    public description = "Requires that you have 5 cards with requirements in play";
    ...
    public getScore(player: Player): number {
        const validCards = player.playedCards.filter((card) => {
            ...
            const hasRequirements =
                card.canPlay && (card.hasRequirements === undefined || card.hasRequirements);
