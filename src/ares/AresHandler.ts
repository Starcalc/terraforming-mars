...
export const OCEAN_UPGRADE_TILES = [
    TileType.OCEAN_CITY,
    TileType.OCEAN_FARM,
...
export class AresHandler {
    ...
    private static earnAdacencyBonus(game: Game, adjacentSpace: ISpace, player: Player): boolean {
        ...
        if (adjacentPlayer === undefined) {
            throw new Error(
                `A tile with an adjacency bonus must have an owner (${adjacentSpace.x}, ${adjacentSpace.y}, ${adjacentSpace.adjacency.bonus}`
            );
        }

        const addResourceToCard = function (
            game: Game,
            player: Player,
            resourceType: ResourceType,
            resourceAsText: string
        ) {
        ...
        const bonusText = bonuses
            .entries()
            .map((elem) => `${elem[1]} ${bonusAsString(elem[0])}`)
            .join(', ');
        const tileText = tileTypeAsString(adjacentSpace.tile?.tileType);
        game.log('${0} gains ${1} for placing next to ${2}', (b) =>
            b.player(player).string(bonusText).string(tileText)
        );
        ...
        game.log('${0} gains ${1} M€ for a tile placed next to ${2}', (b) =>
            b.player(adjacentPlayer).number(ownerBonus).string(tileText)
        );
    ...
    public static afterTilePlacement(
        game: Game,
        player: Player,
        startingResources?: Multiset<Resources | ResourceType>
    ): void {
        ...
        [Resources.PLANTS, Resources.STEEL, Resources.TITANIUM, Resources.HEAT].forEach(
            (resource) => {
                ...
                const cardName = resource === Resources.PLANTS ? CardName.ECOLOGICAL_SURVEY : CardName.GEOLOGICAL_SURVEY;
                    game.log("${0} gained a bonus ${1} because of ${2}", (b) =>
                        b.player(player).string(resource).cardName(cardName)
                    );
                }
            }
        );
        [ResourceType.MICROBE, ResourceType.ANIMAL].forEach((resourceType) => {
            if (
                ...
                game.defer(new AddResourcesToCard(player, game, resourceType, 1));
            }
        });
    }
    ...
    private static computeAdjacencyCosts(
        game: Game,
        space: ISpace,
        subjectToHazardAdjacency: boolean
    ): IAdjacencyCost {
    ...
    public static assertCanPay(
        ...
        const availableProductionUnits =
            player.getProduction(Resources.MEGACREDITS) +
            5 +
        ...
        if (cost.production > 0) {
            throw new Error(
                `Placing here costs ${cost.production} units of production and ${cost.megacredits} M€`
            );
        } else {
    ...
    public static payAdjacencyAndHazardCosts(
        ...
        if (cost.megacredits > 0) {
            game.log('${0} placing a tile here costs ${1} M€', (b) =>
                b.player(player).number(cost.megacredits)
            );
    ...
    public static onTemperatureChange(game: Game, aresData: IAresData) {
        ...
        testConstraint(aresData.hazardData.severeErosionTemperature, game.getTemperature(), () => {
    ...
    public static grantBonusForRemovingHazard(
        game: Game,
        player: Player,
        initialTileType: TileType | undefined
    ) {
        if (game.phase === Phase.SOLAR) {
            return;
        }
        ...
        game.log("${0}'s TR increases ${1} step(s) for removing ${2}", (b) =>
            b.player(player).number(steps).string(tileTypeAsString(initialTileType))
        );
...
function makeSevere(game: Game, from: TileType, to: TileType) {
    ...
    game.log('${0} have upgraded to ${1}', (b) =>
        b.string(tileTypeAsString(from)).string(tileTypeAsString(to))
    );
}
        ...
        () => {
            game.board.spaces.forEach((space) => {
                if (
                    ...
                    space.tile?.tileType === TileType.DUST_STORM_SEVERE
                ) {
            ...
            if (game.phase !== Phase.SOLAR) {
                ...
                game.log("${0}'s TR increases 1 step for eliminating dust storms.", (b) =>
                    b.player(player)
                );
            }
        }
...
function testToPlaceErosionTiles(game: Game, aresData: IAresData, player: Player) {
    testConstraint(aresData.hazardData.erosionOceanCount, game.board.getOceansOnBoard(), () => {
