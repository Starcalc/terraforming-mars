...
describe('AresHandler', () => {
    ...
    it("Get adjacency bonus", () => {
        ...
        game.addTile(otherPlayer, SpaceType.LAND, firstSpace, {
    ...
    it("setupHazards", () => {
        ...
        const spacesWithTiles: Array<SpaceToTest> = game.board.spaces
            ...
            .map((space) => {
                const x: SpaceToTest = { tile: space.tile!, x: space.x, y: space.y };
                return x;
            });
    ...
    it("Can't afford adjacency costs", () => {
        const firstSpace = game.board.getAvailableSpacesOnLand(player)[0];
        firstSpace.adjacency = { bonus: [], cost: 2 };
        game.addTile(otherPlayer, SpaceType.LAND, firstSpace, { tileType: TileType.NUCLEAR_ZONE });
    ...
    it("Placing on top of an ocean doesn't regrant bonuses", () => {
...
function getProduction(player: Player): Map<Resources, number> {
    ...
    [
        ...
        Resources.TITANIUM,
        Resources.STEEL,
    ].forEach((r) => map.set(r, player.getProduction(r)));
    return map;
}
